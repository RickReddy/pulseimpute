...
def load(mean=None, bounds=None, train=True, val=True, test=False, addmissing=False, path=os.path.join("data/pulseimpute_data/waveforms/hcpa_ppg")):

    # Train
    if train:
        X_train = np.load(os.path.join(path, "hcpa_ppg_train.npy")).astype(np.float32)
        if mean:
    ...
    # Val
    if val:
        X_val = np.load(os.path.join(path, "hcpa_ppg_val.npy")).astype(np.float32)
        if mean:
    ...
    # Test
    if test:
        X_test = np.load(os.path.join(path, "hcpa_ppg_test.npy")).astype(np.float32)
        if mean:
...
def modify(X, type=None, addmissing=False):
    ...
    if addmissing:
        ...
        miss_tuples_path = os.path.join("data","pulseimpute_data","missingness_patterns","mHealth_missing_ppg",f"missing_ppg_{type}_10sec.csv")
        ...
        return X, {"target_seq":target, "list_of_missing":list_of_miss}
