...
def load(mean=None, bounds=None, train=True, val=True, test=False, addmissing=False, path=os.path.join("data/pulseimpute_data/waveforms/eegfmri_vu_resp")):
    ...
    # Test
    if test:
        X_test = np.load(os.path.join(path, "eegfmri_vu_resp_test.npy")).astype(np.float32)
        if mean:
