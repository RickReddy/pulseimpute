...
def load(mean=None, bounds=None, train=True, val=True, test=False, addmissing=False, path=os.path.join("data/pulseimpute_data/waveforms/eegfmri_nih_resp")):
    ...
    # Test
    if test:
        X_test = np.load(os.path.join(path, "eegfmri_nih_resp_test.npy")).astype(np.float32)
        if mean:
