...
def eval_heartbeat_detection(imputation, target_seq, input, path, return_stats=False):
    ...
    def get_groundtruths():
        ...
        with open(f'data/pulseimpute_data/waveforms/mimic_{type}/peaks_list.pickle', 'wb') as fp:
            pickle.dump(r_peaks_list, fp)

        return r_peaks_list
    if not os.path.exists(f'data/pulseimpute_data/waveforms/mimic_{type}/peaks_list.pickle'):
        r_peaks_list = get_groundtruths()
    else:
        with open(f'data/pulseimpute_data/waveforms/mimic_{type}/peaks_list.pickle', 'rb') as fp:
...
def eval_cardiac_classification(imputation, path):
    ...
    datafolder = 'data/pulseimpute_data/waveforms/ptbxl_ecg/'
