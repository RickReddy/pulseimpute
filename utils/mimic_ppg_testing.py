...
def load(mean=None, bounds=None, train=True, val=True, test=False, addmissing=False, path=os.path.join("data/pulseimpute_data/waveforms/mimic_ppg_testing")):
    ...
    # Test
    if test:
        X_test = np.load(os.path.join(path, "mimic_test.npy")).astype(np.float32)
        if mean:
...
def modify(X, type=None, addmissing=False):
    ...
    if addmissing:
        ...
        miss_tuples_path = os.path.join("data","pulseimpute_data","missingness_patterns","mHealth_missing_ppg",f"missing_ppg_{type}_vis.csv")
