...
def load(mean=None, bounds=None, train=True, val=True, test=False, addmissing=False, path=os.path.join("data/pulseimpute_data/waveforms/hcpya_ppg")):
    ...
    # Test
    if test:
        X_test = np.load(os.path.join(path, "hcpya_ppg_test.npy")).astype(np.float32)
        if mean:
